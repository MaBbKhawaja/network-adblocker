...
- A dashboard at **http://adblocker.local** with per-device stats, names, pause buttons, editable allow/block lists,
  an on/off switch, and an internet health monitor with an outage log.
...
- Anything away from home. That would need a home VPN on an always-on computer; see *Possible extensions* below.
...
### Step 3 · Flash the board over USB (once)
...
2. Build and flash:
   ```bash
   ./build.sh all         # writes secrets.h from .env, packs the extension, compiles (~2 min), uploads
...
| Symptom | Cause / fix |
|---|---|
| Whole house "connected, no internet" | The board is off and the router has no fallback in DNS Server 2. Plug the board in, or set DNS Server 2 = 1.1.1.1 as in step 5. |
